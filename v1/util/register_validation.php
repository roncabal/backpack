<?php
	function validate($first_name, $last_name, $email_add, $lock_code, $accept_terms){
		...
		if(empty($first_name)){
			$first_name_error = '&bull; Please enter your first name.';
			$first_name_check = false; 
		}elseif(strlen($first_name) > 20){
			$first_name_error = '&bull; First name must be less than 21 characters.';
			$first_name_check = false;
		}elseif(strlen($first_name) <= 2 ){
			$first_name_error = '&bull; First must be greater than 2 characters.';
			$first_name_check = false;
		}else{
		...
		if(empty($last_name)){
			$last_name_error = '&bull; Please enter your last name.';
			$last_name_check = false;
		}elseif(strlen($last_name) > 20){
			$last_name_error = '&bull; Last name must be less than 21 characters.';
			$last_name_check = false;
		}elseif(strlen($last_name) <= 2 ){
			$last_name_error = '&bull; Last name must be greater than 2 characters.';
			$last_name_check = false;
		}else{
		...
		if(empty($email_add)){
			$email_add_error = '&bull; Please enter you email address.';
			$email_add_check = false; 
		}elseif(!preg_match('/^[a-z0-9]+([_\\.-][a-z0-9]+)*@([a-z0-9]+([\.-][a-z0-9]+)*)+\\.[a-z]{2,}$/i', $email_add)){
			$email_add_error = "&bull; Please enter a valid email.";
			$email_add_check = false;
		}else{
		...
		if(empty($lock_code)){
			$lock_code_error = '&bull; Please enter your lock code.';
			$lock_code_check = false;
		}elseif(strlen($lock_code) < 8){
			$lock_code_error = "&bull; Password must be more than 8 characters.";
			$lock_code_check = false;
		}elseif(strlen($lock_code) > 18){
			$lock_code_error = "&bull; Password must be less than 19 characters.";
			$lock_code_check = false;
		}else{
		...
		if($accept_terms == 0){
			$accept_terms_error = '&bull; Accept check box must be ticked to process registration.';
			$accept_terms_check = false;

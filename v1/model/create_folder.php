...
if($folder_name_check == false){
	$response = array('response'=>$folder_name_error);
...
if(file_exists($myDir.$folder_name)){
	$response = array('response'=>"Folder already exist.");
	echo json_encode($response);
	return;
}else{
	...
	if($dir_query){
		$response = array('response' => 'Folder has been successfully created!');
		echo json_encode($response);
		return;
	}
}

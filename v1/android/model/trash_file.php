...
if($file_type == 'folder'){
	...
	$file_to_delete = "../../user_folders/". $getDir .'/'.$file_url .'/' . $file_name . '.'.$file_type;
...
if(rename($file_to_delete, $file_destination)){
	...
	$update = $db->db->query("UPDATE `$getDir` SET file_dir='trash' WHERE file_name like binary '$file_name' AND file_dir like binary '$file_url'");
	...
	$response['error'] = 1;
	$response['error_msg'] = "Unable to delete file.".$file_to_delete;
	echo json_encode($response);
}
?>